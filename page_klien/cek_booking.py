...
def main():
    ...
    if submit: 
        if pilih_tgl: 
            ...
            data_for_pilih_tgl = [data for data in linked_list.display() if data["Tanggal Booking"] == str(pilih_tgl)]

            if data_for_pilih_tgl:
                df_pilih_tgl = pd.DataFrame(data_for_pilih_tgl)
                                
                kolom = ["Nama", "Model", "Paket Service", "Tanggal Booking", "Estimasi Waktu Servis"]
                df_pilih_tgl = df_pilih_tgl[kolom]
